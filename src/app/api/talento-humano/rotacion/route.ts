...
import { getRotacionData, normalizeTalentoFilters } from "@/lib/talento-humano";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getRotacionData(filters);

    return NextResponse.json(data, {
      headers: { "Cache-Control": "private, max-age=60, stale-while-revalidate=120" },
    });
  } catch (error) {
    return handleApiError(error, "No se pudo cargar los datos de rotacion laboral.");
  }
}
