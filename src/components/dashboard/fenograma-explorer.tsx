...
function LifecycleChip({ active, label, onClick }: {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={active
        ? "inline-flex items-center rounded-full border border-slate-700/40 bg-slate-900/20 dark:border-slate-600/40 dark:bg-slate-900/30 px-3 py-2 text-sm text-foreground font-medium"
      ...
      <Card className="starter-panel border-border/70 bg-card/82">
        <CardHeader>
          <div className="flex items-center gap-3">
            <div className="rounded-full bg-slate-900/10 dark:bg-slate-900/20 p-3 text-slate-700 dark:text-white"><Sprout className="size-5" aria-hidden="true" /></div>
